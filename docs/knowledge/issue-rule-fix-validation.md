...
```text
PrintPreset + user-confirmed TargetGeometry
    -> RuleDefinition
        -> IssueInstance
...
| `PAGE.TARGET_SIZE_MISMATCH` | Observed PDF page differs from confirmed Trim Size | Proportional whole-page normalization when ratio-compatible; otherwise manual | Exact target TrimBox + rerun geometry/PPI rules |
