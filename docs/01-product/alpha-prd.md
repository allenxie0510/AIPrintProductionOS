...
4. **规则优先于 AI**：解析器提供证据，规则引擎判定，修复引擎修改，验证器复检；LLM 只解释和辅助决策。
5. **不静默替换字体**：缺少精确字体时不得自动使用相似字体。
6. **按风险分级自动化**：修复只允许 `auto`、`confirm`、`manual` 三种安全等级。
7. **修复后必须复检**：Analyze → Plan → Repair → Analyze Again → Validate → Export。
8. **临时处理**：默认不长期保存 PDF、提取图片或最终生产文件。
9. **目标印刷条件是配置**：CMYK、出血、目标 PPI 和 PDF/X 由版本化 Print Preset 决定。
10. **证据决定声明**：PDF/X 标识和 OutputIntent 只能产生候选状态，不能替代独立验证。
...
每次诊断必须绑定不可变的 Print Preset 版本，至少包含产品类型、目标 PPI、出血、PDF/X 目标和颜色策略。
